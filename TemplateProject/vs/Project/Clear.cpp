...
void Engine::Clear::Init()
{
	...
	text = (new Text("8-bit Arcade In.ttf", 150, game->GetDefaultTextShader()))
		->SetText("GAME CLEAR")->SetPosition(game->GetSettings()->screenWidth * 0.5f - 400, game->GetSettings()->screenHeight - 200.0f)->SetColor(235, 229, 52);

}


void Engine::Clear::Update()
{
	// Set background
